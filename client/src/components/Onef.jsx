...
function Onef(props) {
  ...
  const delett = (id) => {
    console.log(id);
    axios.delete(`http://localhost:3000/api/female/delete/${id}`)
      .then((data) => { props.setRef(true) , props.changeView("Allfemale") })
  ...
  const saveChanges = (id) => {
    axios.put(`http://localhost:3000/api/female/update/${id}`, editedData)
      .then((response) => {props.setRef(true), props.changeView("Allfemale")})
      .catch((err) => {console.log(err)});
  };
